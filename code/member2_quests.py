"""
MEMBER 2: QUEST/CHALLENGE SYSTEM
Author: [Your Name Here]
Description: Handles quest creation, acceptance, completion, and rewards
"""

import os
import sys

# Import Member 1's module (they're in the same directory)
...
def get_all_quests():
    """Read all quests from file and return as list of dictionaries"""
    quests = []
            ...
            for line in f:
                ...
                if len(parts) >= 8:
                    quest = {
                        ...
                        "creator_id": parts[6],
                        "status": parts[7],
                        "participants": parts[8] if len(parts) > 8 else ""
                    }
                    quests.append(quest)
    return quests
...
def create_quest(creator_id):
    """Create a new quest - INTERACTIVE"""
    print("\n" + "="*50)
    ...
    # Save quest
    with open(QUESTS_FILE, "a") as f:
        # Format: id|title|description|reward|type|difficulty|creator|status|participants
...
def complete_quest(student_id, quest_id):
    ...
    for quest in quests:
        if quest['id'] == quest_id:
            ...
            # Give reward by calling Member 1's function
...
def quest_menu(student_id):
    ...
    while True:
        ...
        if choice == "1":
            ...
            view_quests()
            quest_id = input("\nEnter Quest ID to accept (or press Enter to cancel): ").strip().upper()
            if quest_id:
            ...
            quest_id = input("\nEnter Quest ID to complete (or press Enter to cancel): ").strip().upper()
            if quest_id:
...
if __name__ == "__main__":
    print("="*60)
    print("TESTING MEMBER 2: QUEST SYSTEM".center(60))
    print("="*60)
